...
| # | 需求 | 来源 | 层 | 说明 |
|---|---|---|---|---|
| R-01 | **VGGT-Ω 几何重建（L2-S1）** | 商讨 | 二 | 权重已就位并验证；已在 UAVScenes 真实场景多次跑通（24 帧 @512）。**仍缺**：与 LiDAR 逐点比对重建质量（需 M-008） |
| R-13 | L2-S3 提升融合 / L2-S4 派生 | 建议 | 二 | 代码可先按契约写好，接口留给 VGGT-Ω 深度 |
| R-14 | 专家模型产出真实 artifact | 建议 | 二 | 模型已部署但未接线，按 §14.8 I/O 契约存盘 |
| R-28 | **置信度校准**（LiDAR 残差 → 错误事件概率） | 建议 | 二 | 实测两重非平稳：**场景相对**（同类植被跨场景差 2.4 倍）+ **光照相关**（亮度 0.25× 时水面/陆面 AUC 由 0.865 掉到 0.670）。**必须按成像条件分档标定**，分位数阈值在低照度下失效。需 M-008 外参。<br>*（原编号 R-20，与「Qwen 部署」重号，2026-08-25 改号）* |
| R-29 | **成像条件分档字段** | 建议 | 一/二 | 场景快照需带图像亮度/对比度统计，供 R-28 分档与 C4 配对使用。当前配对靠临时脚本算 |
...
|---|---|---|
| **C1** | **感知可信度与失效归因** | LiDAR 与视觉深度的残差可**程序化产出失效标注**。HKisland 约 40% 水面、HKairport 约 52% 均质硬化面提供天然大样本；`*_Evening` 航次尾段与日间同地点同航线（RTK 实测可配到 10 m 内、亮度 0.25×），构成**受控的成像退化对照**。这对应社区头号诉求 —— 飞手最怕的不是损失飞机，而是不知道系统当前是否可信 |
...
| **C4** | **跨时相变化与光照鲁棒** | `*_Evening` 与日间航次配对，几何真值相同。可区分**真实三维变化**与**仅由光照造成的表观差异**。实测还发现更难的一层：模型的**不确定性估计本身**在暗光下失准（判别力 AUC 0.865→0.670），可据此出「你的把握可信吗」这类二阶题，负样本可程序化产出 |
