...
> 「我希望搭的这个数据生成的 pipeline 是对于低空采出来的数据集都能用的，
> 它应该是不会过分依赖数据集本身需要提供的这些 3D 信息的……
> 我们假设数据集只能提供静态的图片或者说视频，那我们可以通过一些模型去提取出这些 3D metadata，
> 是有可能不准确，但是这样的 pipeline 才是有价值的。不然的话你对数据集本身的要求太高了，
> 这个 3D pipeline 就只能是对某一些极为特殊的数据集才适用的方法，这不是我们想要的。」

**偏离的范围（已核查）**：LiDAR 在 SPEC 中出现 27 处，其中数据集登记表 10 处（无害）、
提取主干 §14 仅 2 处。**污染集中在 §40.3 与 §46** —— 即 2026-08-25 上午重新定义
C1–C4 与任务真值表的地方。L2 的提取链路本来就是纯视觉的，未受影响。

**处置**：新增**铁律 14**（target MUST 可由数据集自身视觉信息 + 模型推出；
传感器真值 MAY 用于验证与标定，MUST NOT 作为生产前提），
并据此重写 §40.2、§40.3（C1–C4）、§46.1–46.4、§14.13、§16、§23、§43.2、§51。

**新增 §40.5「五条机制」**（如何从不可靠提取得到可靠 target）：
测量优于观点、序数/角度优于米制、一致性做筛子不做真值、失效即标签、尺度分档。

**新增 §40.6「一次性标尺」**：LiDAR/RTK 的唯一用途是离线测量纯视觉管线的误差
并标定置信度，产出一份带误差棒的验证报告（Release D），而非训练数据。
**直接后果：M-008 从阻塞项降为非阻塞项。**

**被纠正的具体技术错误**：§46.2 原写「坡度由 LiDAR 确定性计算」。
**坡度是角度，在均匀缩放下不变** —— 视觉点云在尺度未知时依然给出正确角度。
C2 的核心几何量本就不需要任何传感器真值。同理粗糙度取比值后也无尺度依赖。

**用户当场拍板的两个岔口**：

1. **泛化验证时机** —— 先在 UAVScenes 上把纯视觉主干跑通并拿到误差刻画，
   再引入第二个纯视觉数据集。理由：UAVScenes 的 LiDAR 正是「一次性标尺」的必要条件。
2. **米制任务处理** —— 按尺度锚分档保留，**默认走 GPS/EXIF（T3）**。
   C3 拆为 C3-a（序数/比值，任何数据集）与 C3-b（绝对米制，T3 及以上），
   无锚数据集静默降级而非报错或伪造。

### 19.9 交付物优先级：任务类型的价值高于语料质量（2026-08-25）

**[用户已确认]** 同日用户进一步明确交付重心：

> 「这种低空的 3D 点云数据应该是没有过的。我们现在的想法是利用手头能用的工具，
> 先做一批目前能达到的比较高质量的数据。但是这个高质量是相对的。
> 所以相较于数据绝对的高质量，我更希望我们提出的下游任务的类型是值得做的。
> 即便我们的数据在这个任务上的质量不是很高，但是这个任务会指明一个方向，
> 有了这个方向后，那就是之后做数据、造数据的人该去进一步完善、去优化的目标。
> 该往哪个方向走，即哪些任务数据能够补充之前训练数据缺失的能力，这个能力是什么，
> 好的数据需要满足什么样的特点，这是我们想要的。」

**处置**：新增 **§1.1 交付物优先级表**（任务类型 > 能力缺口论证 > 好数据判据 >
可移植方法 > 语料本身）与 **§52 任务价值论证与数据演进目标**。

§52 规定每个能力 MUST 回答五问：能力缺口 / 为何该任务能训到 / **当前上限（诚实）** /
**好数据的特征** / 衡量指标。**缺论证的任务不进 Release**，哪怕技术上能生成。
§52.0 另给出「任务是否值得做」的五条通用判据（能力真空、捷径不可解、可判定、真实诉求、可移植）。

**写作纪律的改变**：当前数据的质量上限 MUST 诚实写明，不得用「已足够」掩盖 ——
**写清做不到什么正是「指方向」的一部分**。在「提高本批数据质量」与
「把任务类型论证清楚」冲突时，优先后者。

---

### 19.10 判据由「2D 做不到」改为「3D 增益可测」；项目定位为蒸馏（2026-08-25）

**[用户已确认]** 用户指出前一版判据（§19.9 写入的「捷径不可解」、
铁律 5 的「MUST require 3D information」）在信息论上站不住：

> 「很难去找到那种纯视觉就一点做不了的东西 —— 我们的 3D Point Cloud 也是从纯视觉的
> 数据集经过转化得来的，3D 信息也是专家模型从视觉信息中提取的。从信息论的角度上说，
> 我们得到的信息一定是会有损失的。不是说视觉一定实现不了，而是之前的 VLM 因为它不会
> 专门利用这些 3D metadata，纯视觉输入会忽视掉很多 3D 信息。我们把 3D 信息用目前理解
> 3D 能力最好的专家提取出来，又刻意放到模型里边，所以它应当是有提升，而不是 0 和 1 的
> 区别。当然如果有只有 3D 数据能做、2D 原始输入做不了的，那当然最好，但不要放这么绝对，
> 口子可以放大一些。重要的是这些 3D metadata 的确能对任务起到帮助作用。」

**形式化**：`M = f(I)`（metadata 从图像提取）、`Y = g(I)`（target 也从视觉推出，铁律 14）。
数据处理不等式给出 `I(M;Y) ≤ I(I;Y)` —— **凡是能从 M 回答的原则上都能从 I 回答**。
故「2D 绝对做不到」是不可能达成的门槛。

**处置**：铁律 5 改为 `MUST demonstrate measurable 3D lift`；
§29 由事后验证升格为准入判据并定义 `lift` 与伪增益排除手段；
§0.4、§52.0 判据同步；§52 论证由五问增为**六问**（新增「3D 增益」）。

**这不是放松**：从「断言不可解」（从未验证过）变成
「实测 lift 显著 > 0 **且**打乱/屏蔽后选择性消失」（每条都要跑数字）。

**唯一保留的 2D 真不可解**：尺度锚来自 EXIF/GPS/飞控日志 —— 信息不在像素里，
C3-b 因此是本项目少数真正 2D-不可解的任务，**是加分项不是门槛**。

**项目定位（§0.7）** —— 用户原话：

> 「我们现在自己也不用新的数据集，也没有我自己标注的新的数据集，也没有我自己所新训练
> 提出的模型。那么本质上我们是在蒸馏用到的模型的能力。我们的思路呢，就是博采百家之长。」

据此写入三层不可替代性：单教师蒸馏（上限=该教师）→ 多教师融合（上限=并集）→
**集成结构产生的新信号**（教师分歧、自洽违反、扰动漂移；超出任何单个教师的**输出接口**）。
**C1 位于第三层，这是它成为旗舰的根本原因**，优先级排序与三层一致。

**写作纪律**：MUST NOT 宣称超越现有专家模型的三维能力；
分歧信号 MUST 表述为「超出单个教师的输出接口」，
**MUST NOT** 写成「超出信息论上限」或「创造了新信息」。

**新暴露的冲突（待用户裁决）**：新增 R-43（3D 增益实测）必须调模型，
而 R-20（Qwen 部署与调用）在暂缓区。
「暂缓调模型」与「任务须有增益数字才能进 Release」不能同时成立。

---

### 19.11 项目脉络：3D-Anything 是本项目的前身（2026-08-25）

**[用户已确认]** 用户说明两个项目的关系：

> 「3D-Anything 这个项目呢，就是我最早开始对从 2D 数据里边去提取一些信息，
> 作为 3D 下游任务的尝试。那么现在这个项目呢，就是对 3D-Anything 这个基础项目的
> 进一步拓展。我们要让它实现更多、更有效的下游任务。那么怎么样去做到更有效呢？
> 就需要构建更复杂的数据生成管线，以及引入各种专家模型，来去提取 3D metadata 的特征。
> 那么同时呢，为了进一步的完善，我们在整个 pipeline 设计完之后，去看有哪些能够优化的
> 地方，或者说我们能更好地利用这些模型，所以说就引出了 skill 的这一层。」

**因此三者的关系是**：

| | 3D-Anything（前身） | 3D-data-pipeline（本项目） |
|---|---|---|
| 提取 | 直接让 Qwen 读图 | **引入各类 3D 专家模型**提取 metadata（VGGT-Ω / MoGe / DA3 / 分割 / 光流…） |
| 管线 | 单段：图 → prompt → ShareGPT | **多段**：提取 → 提升融合 → metadata 分层 → 任务编译 → 三路 adapter |
| 任务 | Caption / Dialog / Grounding CoT / Decomposition / VG-QA | 在此之上做 **C1–C4**，且每个任务要过 §52 六问 |
| 产物格式 | ShareGPT | **沿用 ShareGPT**（本项目的格式决策与前身一致） |

**这条脉络解释了第三层（Skill）的正确定位** —— 它印证了 §19.4 的暂缓决定：

> Skill 回答的是「**pipeline 设计完之后**，哪些地方能优化、怎么更好地利用这些模型」。

所以它**天然是后置的**：没有跑起来的管线，就没有「可优化处」可谈。
这不是把它降级，而是它本来就属于第二轮。§19.4 当时的理由（「保障机制应当在被保障的
东西成型之后再固化」）与用户此处的表述是同一件事的两个说法。

**对实施的直接影响**：

1. **复用而非重写** —— Qwen 服务与批量推理客户端直接沿用 3D-Anything 的
   `start_qwen35.sh` / `scripts/infer_qwen35.py`，只按机器配置适配；
2. **3D-Anything 的改动走独立分支**（用户要求：不影响原项目）——
   已建分支 `8xa100-40g`；
3. 本项目与前身的**接口是 ShareGPT + OpenAI 兼容 API**，两边解耦。

---

### 19.12 许可策略：学术非商用即可用；AGPL 顾虑解除（2026-08-25）

**[用户已确认]** 用户原话：

> 「关于你说我需要决策的事，我们的项目是纯为科研服务的，
> 只要是可以为学术用途且不参与商业用途能使用模型的话，就请你用。
> 否则你再给我反馈它的使用限制情况。」

**确立的策略**：

1. **准入线 = 学术非商用可用。** 许可允许学术研究且不要求商业授权的模型，
   **直接采用，不再单独请示**；
2. **不满足的**（要求商业授权、仅评测不得训练、需逐案申请且未获批）
   → 不接入，并把**具体限制条款**反馈给用户，由用户判断是否值得申请；
3. **义务性条款照做，但不构成阻塞** —— 署名、引用、附协议副本、
   演绎作品沿用同一许可，这些都写进发布清单，不必逐条请示。

**M-011（AGPL 决策）据此关闭**，且核验后发现原先的顾虑本身不成立：

> AGPL-3.0 §13 的源码披露义务需要 **(a) 修改了程序 且 (b) 远程用户通过网络与修改版交互**
> 两个条件同时成立。§0 把「在计算机上执行」与「修改私有副本」排除在 propagate 之外；
> §2 无条件允许运行，并写明「运行的输出仅在其内容本身构成受保护作品时才受本许可约束」。

因此**本地离线推理不触发任何 AGPL 义务，产出的数据集不带传染**。
唯一义务在**重分发**：不得把 AGPL 模型的代码或 `.pt` 权重打进公开发布物
（只给上游链接即可完全规避）。PowerLine-MTYOLO / A-YOLOM / ultralytics 三者同理。

**同时确立一条不可逆的发布约束**（这是本次核验最重要的产出）：

> DSINE、UFM（权重）、DAM-3B、SegFormer 四者均为非商业许可，
> 且其中三者（UFM 的 SA、DAM-3B §3.2、SegFormer §3.2）
> **把非商业限制传递给演绎作品**。
> ⇒ **本项目发布的数据集必须是非商业的，事后无法改成宽松许可**，
> 除非把这四个模型全部拆掉重跑。

好在这不压低天花板：UAVScenes 本身即 CC BY-NC-SA 4.0（§19 数据集条），
产出本来就得走 NC-SA，模型侧的 NC 与之重合。

**核验方法**（写下来是因为下次加模型要照做）：一律取**许可原文**——
GitHub `LICENSE` 文件 / license API / HF model API，
**不得凭记忆或凭 HF 标签下结论**。实测两处标签与原文打架：
UAVScenes 的 HF 标签漏了 NC；UFM 的 README 写 CC BY-NC-**SA**、
HF 五个 checkpoint 全标 `cc-by-nc-4.0`（无 SA）。**一律从严。**

详细逐条结论见 `FINDINGS.md`「许可」节；需要用户动作的三项见 `USER_ACTIONS.md`
（DSINE 属人许可 vs 共享服务器、DSINE 论文寄送义务、UFM 许可口径澄清）。

---

### 19.13 Grounded-SAM 必须接入；实测后分工重定（2026-08-25）

**[用户已确认]** 用户原话：

> 「我理解，你现在只配置了 oneFormer，因为航拍主体是类似水面这样的
> 没有不好去划定边界的 stuff，而不是 things。但是我觉得 Grounded Sam 是一定要用的，
> 所以到请找最新的版本，下载权重，然后测试并保证能够跑通。」

**被推翻的判断（我的）**：`pipeline/segmentation.py` 初版写「用 OneFormer 而不是
Grounded-SAM，因为航拍主体是 stuff 不是 things」。
**前半对、后半错** —— stuff 该用语义分割成立，但这**不构成不用 Grounded-SAM 的理由**：
语义图给不出实例，而 grounding 任务族、C2 的人数计数、跨视角对应**都需要个体**。

**版本选择**（用户要求「最新版本」，核验结论）：

- `IDEA-Research/Grounded-SAM-2` v1.0（2024-12）**仍是该系列最新 tagged release**；
  初代 Grounded-Segment-Anything 用 SAM 1，已被取代；
- Grounding DINO **1.5 / 1.6 Pro** 更强但**只有 API、不放权重** → 离线管线用不了；
- 走 **transformers 原生实现**而非 clone 官方 repo：后者要编译 CUDA 算子 `_C`，
  会再搅一遍本已脆弱的 numpy/torch 依赖（规则 3）。架构与权重一致，差别只在 runtime；
- **SAM 3**（2025-11）是另一条路线（单模型概念提示分割），许可已核可用，另行评估，
  不算作「Grounded-SAM 的最新版」。

**实测后重定的分工**（这是本次交付的核心结论，依据见 FINDINGS「许可」上方两条）：

| | 边界（哪一个） | 类别（是什么） |
|---|---|---|
| Grounded-SAM-2 | ✅ 逐实例 mask | ❌ **负控实测零判别力** |
| OneFormer | ❌ 无实例 | ✅ 可信 |

⇒ **边界取 Grounded-SAM，类别取 OneFormer**，在 `pipeline/instance_fusion.py` 融合；
GDINO 的短语只作 `label_proposed` 留在血缘里，**MUST NOT 进 `category`**。

这不是妥协而是 §0.7 第二层的融合，且两者的**分歧**（`category_agrees`，实测一致率 38%）
是第三层信号 —— 单个教师给不出「我这个实例的类别有争议」。
