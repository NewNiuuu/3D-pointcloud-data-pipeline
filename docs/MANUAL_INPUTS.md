...
| M-008 | **UAVScenes `calibration_results.py`（相机-LiDAR 外参）** | 验证各深度模型的米制尺度是否可信 —— 需把 LiDAR 投影到图像与预测深度逐像素比对 | `data_raw/UAVScenes/` | 待提供 | 2026-08-24 | **不在 interval=5 档案中**，只在 OneDrive/Google Drive 的完整版根目录。没有它，所有模型的 `domain_calibrated` 只能保持 false，绝对米制任务无法解锁（SPEC §14.11） |
...
- **M-007 与 M-008 是当前的阻塞项**：前者卡住几何重建，后者卡住米制尺度验证。
