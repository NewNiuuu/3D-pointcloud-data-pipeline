...
本仓库配置了 GitHub remote：

```
origin    https://github.com/WoodSerenity/uavlm.git
```
...
|---|---|---|---|---|---|---|
| M-005a | **UAVScenes 下载凭证 / 许可申请结果** | Layer 1 小样本下载 —— **当前关键路径**，卡住则整个 Phase 1 无法开工 | 待定，见 §4 数据集条目模板 | 待提供 | 2026-08-24 | 已确定为首批唯一数据集。多数 UAV 数据集需申请表或机构邮箱，**建议尽早提交申请**；本机无任何原始数据可替代 |
| M-004 | 飞书数据集清单最新只读导出 | Layer 1 重新核对去重，更新 `feishu_snapshot` | `registry/feishu_snapshot/<date>.yaml` | 待提供 | 2026-08-24 | **只读**。文档 `Oc5Owx8hoifPGikC5ZgcxRzGnDd` 需登录，Agent 无法直接访问；不得修改飞书原文档 |
| M-002 | HuggingFace Token | 下载 gated 权重（SAM 2.1、Grounding DINO、DINOv2、MoGe-3 等） | `HF_TOKEN` | 待提供 | 2026-08-24 | 部分 gated 仓库还需先在网页端接受协议。到 vertical slice 第 4–5 步才需要 |
| M-001 | GitHub Token（或 SSH key） | 推送 `3D-data-pipeline/` 到 `NewNiuuu/3D-pointcloud-data-pipeline` | `GITHUB_TOKEN` 或 `secrets/.env.local` | **待提供（阻塞中）** | 2026-08-24 | 本地仓库已初始化、已提交、remote 已配置，**仅差认证**。远程仓库为空。建议用 fine-grained PAT 只授权该仓库的 Contents:write |
...
- **M-005a 是唯一的硬阻塞项。** 其余条目都在其之后的环节才触发。
- M-005a 与 M-004 建议同时启动：前者等待周期最长（外部审批不可控），后者只需你导出一次。
...
- **本文件只管「需要用户手动提供的外部信息」。**
- 需要用户做**技术决策**的未决项（首批数据集、是否强制 metric scale、首批任务、Qwen 部署方式、质量阈值等）**不放这里**，它们在 `CLAUDE_CODE_PROJECT_SPEC.md` §36 `Unresolved Decisions` 和 `README.md` 的「开始实现前需要确定」中维护。
