...
### 不用 metric 模型也能拿到米制深度 —— 相对深度乘一个数就行

多视角重建虽然差一个 7 自由度的相似变换，但**只有其中 1 个标量（尺度）影响米制量**，旋转和平移不改变任何两点间距离。所以只要有**一个**外部米制约束就能锚定整个重建。

我们手上就有：UAVScenes 每个 run 带 RTK，相机轨迹的真实米制位移已知。

```
s = |真实相机位移| / |重建相机位移|
```

实测把 DA3 的相对深度乘上这个 `s`，最好情况与 LiDAR 真值中位数**只差 0.5%**。

**为什么重要**：**不必依赖模型自称 metric**。而且这一步**不需要相机-LiDAR 标定文件** —— 两边比的都是相机位置，所以 M-008 不再是米制能力的阻塞项。
...
**为什么重要**：基线太短时轨迹退化，尺度这个自由度根本没被约束住。Task Spec 必须加**相机基线下限**才能出米制题。

### 而且对齐残差小并不代表尺度可信

上表里 0.15 m 那一行的对齐残差是三者中**最小的**（0.026 m），尺度误差却是 77%。

**为什么重要**：这是个很容易上当的陷阱 —— 退化轨迹本来就容易拟合得好，残差小只说明这几个点能对上，不说明尺度被有效约束。**判断尺度可不可信要看基线长度和轨迹构型，不能看残差。**

📄 `SCALE_RECOVERY_ANALYSIS.md`
