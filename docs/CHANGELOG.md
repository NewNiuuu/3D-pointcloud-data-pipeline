...
### `[需求]` README 要有一张「已跑通的模型」表 — 用户

用户原话：「在 README 文档中，专门维护一个现在已经跑通了的模型表」。

新增 `README.md` §「🤖 已跑通的模型」，10 行（9 个模型 + Grounded-SAM-2 组合卡下挂 2 个组件）。
**表的设计要点是把「跑通」拆成三级**：部署跑通 / 航拍验证 / **已接线产出 artifact**。
此前 README 只说「7 个模型已部署验证」，看不出哪些真正在管线里跑 —— 实际只有
VGGT-Ω 与 OneFormer 两个接了线，其余 7 个都停在「能跑但没产出」。第三列就是为了让这件事一眼可见。

一并附了两条不能忘的约束（Grounded-SAM 只出边界；非商用许可锁死发布物）
与未部署清单，避免用户误以为表外的模型不存在。

涉及文件：`README.md`。

### `[修正]` README / OPERATIONS 三处状态过期 — Agent

改表时核对专家卡与代码，发现三处与事实不符：

| 位置 | 原写法 | 事实 |
|---|---|---|
| README L2-S2 | 「7 个模型已部署验证，尚未产出 artifact」 | 9 个已部署；且 OneFormer 已接进 `pipeline/extract.py` |
| README L2-S3 | 「⬜ 未开工」 | `pipeline/instance_fusion.py` 已实现（DESIGN §14.1.2），2D→3D 提升仍未开工 |
| OPERATIONS 总览 | VGGT-Ω「代码就绪，**权重待批**（M-007）」 | M-007 已于同日解决，权重 4.3 GB 落地并多次跑通 |

第三条是 M-007 关闭时只改了 `USER_ACTIONS.md` 没回头改 OPERATIONS 留下的。
已在 OPERATIONS 总览表上方加了指针，注明它与 README 那张表**必须同步改**，
两份职责不同（一份记部署事实与复现，一份给用户做追踪）。

涉及文件：`README.md`、`docs/OPERATIONS.md`。
...
**这是对的，而且是我写漏的。** §40.1 只写了「MUST NOT 在本数据集上生成这些任务」，
却让读者（包括我自己）连带推出「相关专家模型也等以后再说」。**两件事的门禁不同**：

| | 是否受数据集限制 | 现在做不做 |
|---|---|---|
| **生成**导航类任务样本 | 受限 | ❌ 不做（铁律 5） |
| **准备**相应专家模型 | **不受限** | ✅ **现在就做** |

**为什么不能等**：许可核验与部署验证**会失败且失败隐蔽**，本项目已踩过三次
（DA3NESTED 是 NC 许可、OneFormer 疑似加载错误、SEA-RAFT 权重不在 HF）。
等拿到数据集才发现专家不可用，等于把两件可并行的事白白串行。

**处置**：§40.1 加说明框；新增 **§46.6 专家模型提前准备清单**（与 §46.5 的能力清单分开门禁），
规定准备工作必须做完四项：许可核验（代码与权重分别核）、可获取性、部署验证、输出契约实测。

### `[实现]` R-14：语义来源改为模型，铁律 14 的可移植性补齐 — Agent

**动机**：`extract_l1` 原先直接消费 UAVScenes 的逐像素人工标注 ——
在本数据集省事，但换个没有语义标注的数据集就断了，**方法不可移植**，正是铁律 14 要防的。

**新增 `pipeline/segmentation.py`**（OneFormer 封装）：

- **选 OneFormer 不选 Grounded-SAM**：航拍主体是水面/植被/裸地/硬化面这类
  **stuff**（无定形、不可数），Grounding DINO 出框，对 stuff 不好使；
- **不直接用 ADE20K 的 150 类**，按下游需要归并到 10 个项目类别（§0.3 反向证成）——
  C2 判可降落必须区分水面/植被/硬化面。原始标签保留备查；
- `extract_l1` 新增 `semantic_source="native"|"model"`。
  **保留 native 不是冗余，是对照组** —— 同场景两条路各跑一遍即可量化「换模型损失多少」。
...
| AMtown02_0000 | 12（`cls1`/`cls2`/`cls19`…） | **20**（building/ground_paved/vehicle…） |
| GNSS01_0041 | 8 实体 | 12 实体 |

**换模型不是降级。** AMtown 的人工标注给的是我们从没反查过的类别 ID，
模型给的是**直接对应 C2 判据**的类别。（**注意**：未对真值验证过，
只能说更**可用**，不能说更**准**。）吞吐约 1.4 s/帧。

**抓到一个安全问题**：类别映射第一版把 `person` 归进了 `other`。
C2 的核心判据是「平坦 ≠ 可降落：水面平、车顶平、**人群上方也平**」——
把人归进 other 等于把最危险的降落区放进候选。已单列为硬排除类；
台阶/长椅/雕塑等街道家具同样从 other 移进 structure。

### `[修正]` OneFormer 的加载告警已验证为无害 — Agent

**旧结论被推翻**：此前记「OneFormer 能跑但输出可能是错的，确定兼容版本前不启用」，
理由是 `transformers 5.15.1` 加载时报 `swin.layernorm.weight/bias` MISSING。
**当时并未验证。**

**验证方法**：给该模块挂 forward hook，再把权重强行改成 7.0 / -3.0，比对输出。
**结果**：hook 触发 1 次（模块确实被调用），但 `class_queries_logits` 与
`masks_queries_logits` 的最大变化**恰为 0.000e+00** ——
OneFormer 消费的是 Swin 特征金字塔的中间层，该末端 layernorm 的结果被丢弃。

**教训**：**「加载报 MISSING」不等于「输出错」。** 判据应是「改动该权重是否改变输出」，
而不是日志里有没有告警。当初保守标注是对的，但**拖了太久没去验** ——
期间一直当它不可用，白白挡住了 R-14。状态升为 `deployed_verified`。

### `[实现]` 导航类专家调研：生态里没有可用的预训练专家 — Agent

按 §46.6 对薄结构与空中目标两类做了调研，**结论是负面的但很重要**：

HF 上以 powerline / transmission line / cable / drone detection / airborne object
检索，命中的**全是零下载量的个人习作**；搜 `ttpla` 命中的是同名无关的
Stable Diffusion ControlNet。**生态里现成的是数据集，不是专家**：

| 能力 | 数据集 | 规模 | 许可 |
|---|---|---|---|
| 薄结构 | TTPLA | 1100 张 3840×2160，8987 实例 | HF 镜像标 `other`，**待人工核验** |
| 空中目标 | AOT（Amazon Prime Air） | 4943 段序列、590 万帧，**含距离标注** | 待核验 |
| 空中目标 | AOD4 | 22516 张、4 类 | 待核验（Roboflow，非稳定依赖） |

**这改变了排期的性质**：「准备这两个专家」**不是「下载并测试」，而是「拿公开数据集微调检测器」**，
工作量差一个量级。已在 §46.6 与专家卡里写明，**估期不得按「下个模型跑一跑」算**。

**一个机会**：AOT 带**距离**标注，可能直接支撑 R-34 的 TTC 真值，
不必依赖我们自己的深度估计。引入时应优先核验。

已建两张专家卡（`deployment_status: not_available` —— 不是「没做」，是「查过，没有」）。

**涉及文件**：新增 `pipeline/segmentation.py`、
`registry/experts/{thin_structure_detector,airborne_object_detector}.yaml`；
改 `pipeline/extract.py`、`pipeline/__init__.py`、
`registry/experts/oneformer_ade20k_swin_large.yaml`、
`docs/DESIGN.md`（§40.1 说明框、§46.6）、`docs/FINDINGS.md`、`README.md`。233 项测试通过。


### `[修正]` `[运维]` 幽灵 vLLM 占卡 — 用户发现

**用户通过 nvitop 发现**：模型用完之后 4 张卡显存仍未释放、利用率却很低，
判断是 vLLM 服务没关。**属实，是我的疏忽** —— 跑完 R-43 消融后没停服务，
4 卡各 35.7 GB 被占约 40 分钟。

**为什么容易漏**：vLLM 空转时**显存照样是满的**，GPU 利用率接近 0，
两个最顺手的观察量都区分不出「在服务」与「闲置」。别人看 nvitop
只看到「卡满、算力闲」，既用不了也不知道该找谁。

**即时处置**：`kill -TERM` 主进程（**不用 `pkill -f`** —— `-f` 匹配完整命令行，
会把执行该命令的 shell 一起杀掉，本项目踩过），20 秒内优雅退出，
8 卡全部归零；随即 `gpu_guard.sh once` 把占卡程序拉起来。

**治根因**（新增到 `3D-Anything` 的 `8xa100-40g` 分支）：

| 文件 | 作用 |
|---|---|
| `stop_qwen35.sh` | 优雅停 + **校验显存真的释放** + 自动挂回占卡守护 |
| `watch_qwen35_idle.sh` | 空闲看门狗，默认 30 分钟无请求自动停 |

**看门狗的判据是累计请求数，不是显存或利用率** —— 理由同上。
读 vLLM `/metrics` 的 `vllm:request_success_total`，计数器不涨即真闲置。

**顺手修掉停止脚本自己的一个假阳**：首版的残留检查把占卡程序
（`thinking.py`，每卡 27 GB）报成了「vLLM 残留 worker」，
会误导人去 kill 它。已排除。

**涉及文件**：`3D-Anything`（分支 `8xa100-40g`）：`stop_qwen35.sh`、
`watch_qwen35_idle.sh`、`start_qwen35_8x40g.sh`（输出加提示）、
`CLAUDE.md`、`docs/progress_log.md`；本仓库 `CLAUDE.md` 规则 2 新增
「常驻推理服务用完必须显式停」一节。

**为什么记这条**：这是共享机器上的**协作问题**，不是技术问题 ——
一个人忘记停，所有人都用不了卡。规则里原先只写了占卡程序的协议，
没覆盖「自己起的常驻服务」这一类。
