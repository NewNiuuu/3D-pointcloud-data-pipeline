...
| M-011 | ~~**AGPL-3.0 的使用决策**（PowerLine-MTYOLO / A-YOLOM 分叉 runtime）~~ | 薄结构专家（R-30） | 决策记入 `DECISIONS.md` §19.12 | **✅ 已解决** | 2026-08-25 | 用户确立策略「学术非商用可用即用」。且核验许可原文后发现**原顾虑本身不成立**：AGPL §13 需「修改了程序」+「远程用户经网络交互」两个条件同时成立，本地离线推理一个都不占；§2 明写运行输出不因运行而受许可约束 → **产出数据集不带 AGPL 传染**。唯一义务是不得把代码/`.pt` 权重打进公开发布物（给上游链接即可） |
| M-012 | **DSINE 的许可与共享服务器冲突 —— 需你决定怎么处理** | DSINE 是表面法向专家（尚未接入） | 决策记入 `DECISIONS.md` | **待你决定（不阻塞，DSINE 尚未接入）** | 2026-08-25 | 帝国理工的自定义许可 §2(a)/(e)：**属人许可**，限装在**一台**机器上，且「**须确保软件不被其他用户访问，除非他们自己也接受了本协议**」。本服务器是多人共用的（规则 2）。三条出路：① 装在其他用户读不到的路径下；② 让同机协作者各自接受协议；③ 申请多用户/站点许可 —— 邮件 `researchcontracts.engineering@imperial.ac.uk`。**不选就先不接入 DSINE** |
| M-013 | **DSINE 的论文寄送义务（发表时触发）** | Release 时的合规动作 | 发布清单 | **待办（发表前）** | 2026-08-25 | 许可 §2(f) 允许公开用 DSINE 得到的结果，但要求三件事同时做到：注明 DSINE 是数据来源并附其版权声明、声明「本软件由帝国理工授权使用」、**向帝国理工提交一份该出版物的副本**。第三条是主动动作，最容易漏。仅当实际接入 DSINE 且发表时才触发 |
| M-014 | **UFM 许可口径打架，建议向作者要个书面确认** | UFM 是光流/对应专家（尚未接入） | 决策记入 `DECISIONS.md` | **待你决定（不阻塞）** | 2026-08-25 | 代码是 BSD-3（无问题），**权重**的口径两处不一致：README 写 CC BY-NC-**SA** 4.0，HF 上五个 checkpoint 全标 `cc-by-nc-4.0`（**无 SA**）。我们按**从严**处理（当 SA 用）。SA 有牙齿：若我们生成的对应数据被认定为权重的「演绎作品」，那部分数据就得按 BY-NC-SA 发布。作者正在考虑改 Apache-2.0（`UniFlowMatch/UFM` issue #1）—— 可考虑去顶该 issue 或直接发邮件问 |
